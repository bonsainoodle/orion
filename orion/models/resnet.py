...
class BasicBlockSiLU(on.Module):
    """BasicBlock with SiLU activation instead of ReLU"""
    expansion = 1
...
class ResNetSiLU(on.Module):
    """ResNet with SiLU activation instead of ReLU"""
    def __init__(self, dataset, block, num_blocks, num_chans, conv1_params, num_classes):
        super().__init__()
        self.in_chans = num_chans[0]
        self.last_chans = num_chans[-1]
...
def ResNet20SiLU(dataset='cifar10'):
    conv1_params, num_classes = get_resnet_config(dataset)
    return ResNetSiLU(dataset, BasicBlockSiLU, [3,3,3], [16,32,64], conv1_params, num_classes)

def ResNet18SiLU(dataset='imagenet'):
    conv1_params, num_classes = get_resnet_config(dataset)
    return ResNetSiLU(dataset, BasicBlockSiLU, [2,2,2,2], [64,128,256,512], conv1_params, num_classes)

def ResNet34SiLU(dataset='imagenet'):
    conv1_params, num_classes = get_resnet_config(dataset)
    return ResNetSiLU(dataset, BasicBlockSiLU, [3,4,6,3], [64,128,256,512], conv1_params, num_classes)
